...
class BottomWaypoint extends React.PureComponent {
  ...
  _handleScroll = () => {
    ...
    const waypointNode = this.node.current
    const waypointOffsetTop = waypointNode.offsetTop
    ...
    if (this.node.current.offsetTop < this.scrollableAncestor.offsetHeight) {
      currentPosition = INSIDE
    } else if (
      scrollableParentOffsetTop + scrollableParentHeight >
      waypointOffsetTop - 200
    ) {
      currentPosition = INSIDE
    } else {
